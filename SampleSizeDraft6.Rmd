...
In this example, `r sum(results$typeIIerror)` samples out of 1000 failed to reject the null hypothesis (p>0.05) despite there being a treatment effect, so $\beta =$ `r sum(results$typeIIerror)/1000` . Thus, you can expect that if you choose 200 students for the randomized study and the true effect size is 0.2, the power would be `r 1-sum(results$typeIIerror)/1000`. This power is lower than the desired 0.8. 
...
- Steps 4 & 5: Use a multilevel model to estimate your treatment effect (you can use the package `lme4`)
    ...
    ```{r multi45, evaluate = FALSE, error = FALSE}
